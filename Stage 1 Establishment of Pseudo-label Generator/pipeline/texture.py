...
from pipeline import laws_texture
...
def extract_features_eyes(landmarks, face_crop, scale=256):
    ...
    # energy_features = cv2.calcHist(face_crop,-1,eye_mask, [256], [0,255])
...
def process_faces(face_crop_list, landmarks_list, scale=256):
    ...
    features_0 = extract_features_mouth(landmarks, face_crop, scale=scale)
    # # # ## F2F
    features_1 = extract_features_nose(landmarks, face_crop, scale=scale)
    ...
    # if features_2 is not None and features_3 is not None:
    #     # combine to single feature vector
    #     feature_vector = np.concatenate((features_2, features_3))
    # if features_0 is not None and features_1 is not None and features_2 is not None and features_3 is not None:
    #     # combine to single feature vector
    #     feature_vector = np.concatenate((features_0, features_1, features_2,features_3))
    # if features_3 is not None:
    #     # combine to single feature vector
    #     feature_vector = features_3
