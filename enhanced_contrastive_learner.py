...
def args_func():
    ...
    parser.add_argument('--weight_decay', type=float, default=1e-4,
                        help='weight decay')
    parser.add_argument('--select_confidence_sample', type=float, default=1e-4,
...
def set_loader(pseudo_label_dict, args):
    ...
    train_dataset = Dataset(args.data_folder, pseudo_label_dict, TwoCropTransform(train_transform))
    ...
    val_dataset = Dataset(args.pseudo_label_file, args.data_folder, val_transform)
...
def set_model(args):
    ...
    if torch.cuda.is_available():
        if torch.cuda.device_count() > 1:
            model.encoder = torch.nn.DataParallel(model.backbone)
        model = model.cuda()
...
def save_path(args):
    model_path = './save/SupCon/{}_models'.format(args.data_folder)
    tb_path = './save/SupCon/{}_tensorboard'.format(args.data_folder)

    save_time = str(datetime.datetime.now())

    model_name = '{}_{}_{}_lr_{}_bsz_{}'.\
        format(save_time, args.dataset, args.backbone, args.learning_rate, args.batch_size)
...
def main():
    ...
    pseudo_label_dict = {entry["image_name"]: entry["new_label"] for entry in json_data}

    # build data loader
    train_loader, val_loader = set_loader(pseudo_label_dict, args)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % args.select_confidence_sample == 0:
            confidence_label_dict = select_confidence_sample(model, val_loader, args)
