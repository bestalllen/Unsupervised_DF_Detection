...
def args_func():
    ...
    parser.add_argument('--face_detector_path', type=str, help='The path of shape_predictor_68_face_landmarks.dat.',
                        default=None)
...
def main():
    ...
    pseudo_video = [{"video name": video, "video label": label} for video, label in video_pseudo_labels.items()]
    new_data = []

    for entry in pseudo_video:
        ...
        # Iterate through image file names and create new dictionary entries
        for image_name in video_image_names:
